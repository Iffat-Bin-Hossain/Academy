...
const StudentGrades = ({ courseId, user, onShowMessage }) => {
  ...
  const getGradeStatusIcon = (assignment) => {
    if (!assignment.hasGrade) {
      if (assignment.notSubmitted) return { icon: '❌', text: 'Not Submitted', color: '#ef4444' };
      if (assignment.gradesPending) return { icon: '⏳', text: 'Pending Review', color: '#f59e0b' };
      ...
      <div style={{ 
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '1rem' }}>
          <div style={{ padding: '1rem', backgroundColor: '#f8fafc', borderRadius: '8px' }}>
            <h4 style={{ margin: '0 0 0.5rem 0', color: '#475569' }}>Overall Average</h4>
